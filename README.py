...
class Readme:
    ...
    STARS_WRAPPER: str = "*" * 80
    STARS_FILE_SEPARATOR: str = "*" * 30
    ...
    @classmethod
    def update(cls):
        file_readme: pathlib.Path = pathlib.Path("README.md")
        file_readme_text: str = file_readme.read_text().strip()
        file_readme_splited: List[str] = re.split(pattern=cls.SEPARATOR_PATTERN, string=file_readme_text, maxsplit=1)
        if len(file_readme_splited) > 1:
            file_readme.write_text(file_readme_splited[0])

        # =============================================================================================================
        dirpath: pathlib.Path = pathlib.Path("examples")
        ...
        with file_readme.open("a") as file_readme_append:
            file_readme_append.write(cls.LINE_EXAMPLES_START)

            for index, file in enumerate(files, start=1):
                file_readme_append.write(cls.LINE_SEPARATOR_FILE)
                file_readme_append.write(f"### {index}. {file.name}\n")

                file_readme_append.write(cls.LINE_FILE_HEADER)
                file_readme_append.write(f"{file.read_text().strip()}\n")
                file_readme_append.write(cls.LINE_FILE_FOOTER)
            file_readme_append.write(cls.LINE_EXAMPLES_FINISH)
